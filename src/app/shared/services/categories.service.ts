...
export class CategoriesService extends BaseService {
    ...
    update(id: number, entity: Category) {
        return this.http.put(`${environment.apiUrl}/api/categories/id?id=${id}`, JSON.stringify(entity), { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }

    getDetail(id) {
        return this.http.get<Category>(`${environment.apiUrl}/api/categories/id?id=${id}`, { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }
