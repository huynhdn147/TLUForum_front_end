...
export class KnowledgeBasesService extends BaseService {
    ...
    getDetail(id) {
        return this.http.get<KnowledgeBase>(`${environment.apiUrl}/api/knowledgeBases/id?id=${id}`, { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }
    ...
    delete(id) {
        return this.http.delete(environment.apiUrl + '/api/knowledgeBases/id?id=' + id, { headers: this._sharedHeaders })
            .pipe(
                catchError(this.handleError)
            );
    }
