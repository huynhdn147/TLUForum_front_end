...
export class UsersService extends BaseService {
    ...
    update(id: string, entity: User) {
        return this.http.put(`${environment.apiUrl}/api/users/id?id=${id}`, JSON.stringify(entity), { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }

    getDetail(id) {
        return this.http.get<User>(`${environment.apiUrl}/api/users/id?id=${id}`, { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }

    getAllPaging(filter, pageIndex, pageSize) {
        return this.http.get<Pagination<User>>(`${environment.apiUrl}/api/users/keyWord?pageIndex=${pageIndex}&pageSize=${pageSize}&filter=${filter}`, { headers: this._sharedHeaders })
    ...
    delete(id) {
        return this.http.delete(environment.apiUrl + '/api/users/id?id=' + id, { headers: this._sharedHeaders })
            .pipe(
                catchError(this.handleError)
            );
    }
