...
export class FunctionsService extends BaseService {
    ...
    update(id: string, entity: Function) {
        return this.http.put(`${environment.apiUrl}/api/functions/id?id=${id}`, JSON.stringify(entity), { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }

    getDetail(id) {
        return this.http.get<Function>(`${environment.apiUrl}/api/functions/id?id=${id}`, { headers: this._sharedHeaders })
            .pipe(catchError(this.handleError));
    }

    delete(id) {
        return this.http.delete(environment.apiUrl + '/api/functions/id?id=' + id, { headers: this._sharedHeaders })
            .pipe(
